...
replacing the 8 hand-written curl+jq implementations from Level 0.

Maps knowpile's own backend names (see core.config.SUPPORTED_BACKENDS) to
...
# One sensible default model per backend. Overridable via Config.model,
# which is what `knowpile config show` / a future `set-model` command edits.
...
def complete(prompt: str, backend: str, model: Optional[str] = None) -> LLMResult:
    ...
    try:
        response = litellm.completion(
            ...
            num_retries=3,  # litellm's built-in exponential backoff
        )
