"""
`knowpile doctor` -- fail fast, never auto-install.

Every check here maps to a real Level-0 failure:
- markitdown is now a library import, so a broken install surfaces as a
  normal ImportError here, instead of a silent 0-byte report file mid-run.
- graphify/cloc/git remain external binaries knowpile depends on but
  doesn't own -- checked here, never auto-installed.
- The Apple Silicon / Rosetta arch-mismatch check is ported directly: it's
  the exact root cause of the original 0-byte-report bug, and it's cheap
  to catch here instead of downstream.
"""
...
def _check_markitdown() -> CheckResult:
    try:
        ...
        return CheckResult(
            ...
            f"Not importable ({e}). Fix: pip install 'markitdown[all]'",
        )
