...
the traceability record every derived fact in the eventual Layer-2 file must
trace back to.
...
proven necessary against a real project. Add it only if a real run shows the
flat structure is actually insufficient -- not before.
"""
...
from knowpile.core.utils import slugify, strip_shell_quotes
...
class ReadmeInfo(BaseModel):
    ...
    def _status_matches_path(self) -> "ReadmeInfo":
        # Bug this guards against: a caller passing status="collected" with
        # a path that cleans down to None (empty, "null", quote-only, etc.)
        # would otherwise leave status and path disagreeing with each
        # other -- derive status from the final path, never trust it as a
...
class ArchDiagramInfo(BaseModel):
    ...
    @classmethod
    def _clean_path(cls, v: Optional[str]) -> Optional[str]:
        # Guards the exact bug found in the pasted example manifest: the
        # JSON string "null" is not the same thing as JSON null, and would
        # otherwise be stored as a literal path named "null".
...
def create_manifest(
    ...
    gathers these same arguments one questionary prompt at a time; a future
    web interface would gather them from one HTTP request body. Neither
    interface touches Pydantic or the manifest schema directly -- they just
    collect these primitives and call this function.
    """
